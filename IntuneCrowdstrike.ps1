...
function Get-Access-Token {
    $json=$(Invoke-WebRequest -Method POST -Body "client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}" -uri ${BASE_URL}/oauth2/token | ConvertFrom-Json)
    echo $json.access_token
}

function Get-SHA256 {
    $json=$(Invoke-WebRequest -Headers @{"Authorization" = "Bearer $(Get-Access-Token)"} -uri ${BASE_URL}/sensors/combined/installers/v1\?filter=platform%3A%22windows%22 | ConvertFrom-Json)
    echo $json.resources[0].sha256
}

function Get-CCID {
    $json=$(Invoke-WebRequest -Headers @{"Authorization" = "Bearer $(Get-Access-Token)"} -Uri ${BASE_URL}/sensors/queries/installers/ccid/v1 | ConvertFrom-Json)
    echo $json.resources
}

# check for install
if (!(Test-Path "C:\Program Files\CrowdStrike\CSFalconService.exe") -and (!(Get-Service -Name csagent).Status -ne 'Running')) {
    Invoke-WebRequest -OutFile c:\Windows\Temp\CrowdstrikeInstaller.exe -H @{"Authorization" = "Bearer $(Get-Access-Token)"} -Uri ${BASE_URL}/sensors/entities/download-installer/v1?id=$(Get-SHA256)
